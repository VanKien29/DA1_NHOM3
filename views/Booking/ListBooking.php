            ...
            <div class="filter-bar mb-3">
                <form method="GET" class="filter-form d-flex gap-2 align-items-center">
                    ...
                    <select name="status" class="filter-input">
                        ...
                        <option value="sap_dien_ra" <?= (isset($_GET['status']) && $_GET['status'] == 'sap_dien_ra') ? 'selected' : '' ?>>
                            Sắp diễn ra
                        </option>
                        <option value="dang_dien_ra" <?= (isset($_GET['status']) && $_GET['status'] == 'dang_dien_ra') ? 'selected' : '' ?>>
                            Đang diễn ra
                        </option>
                        <option value="cho_xac_nhan_ket_thuc" <?= (isset($_GET['status']) && $_GET['status'] == 'cho_xac_nhan_ket_thuc') ? 'selected' : '' ?>>
                            Chờ xác nhận
                        </option>
                        <option value="da_hoan_thanh" <?= (isset($_GET['status']) && $_GET['status'] == 'da_hoan_thanh') ? 'selected' : '' ?>>
                            Đã hoàn thành
                        </option>
                        <option value="da_huy" <?= (isset($_GET['status']) && $_GET['status'] == 'da_huy') ? 'selected' : '' ?>>
                            Đã hủy
                        </option>
                    </select>
                    ...
                    foreach ($bookings as $b):
                        ...
                        <td><?= htmlspecialchars($b['vehicle_name']); ?></td> -->
                            ...
                            <td>
                                ...
                                <a href="?action=admin-updateBooking&id=<?= $b['booking_id']; ?>" class="btn btn-sm btn-outline-success booking-edit-btn 
                                    <?= ($b['status'] == 'sap_dien_ra') ? '' : 'soft-blue disabled-link' ?>">
                                    Sửa
                                </a>
